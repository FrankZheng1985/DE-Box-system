...
1. **踩坑记录**：每次修复 bug 或解决问题后，必须在 `docs/踩坑经验库/` 新建一篇记录，格式参照已有文件（问题现象 → 根本原因 → 错误代码 → 正确代码 → 防护规则），编号递增。
2. **开发记录**：每次开发完成后，在 `docs/开发记录/` 创建文档，包含修改文件路径、功能说明、影响分析。
3. **CLAUDE.md 更新**：如果发现新的架构规则或关键约束，同步更新本文件的"关键架构规则"章节。
