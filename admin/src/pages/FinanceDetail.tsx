    ...
    return (
      <div className="p-4 lg:p-6">
        <button onClick={() => navigate('/finance')} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 mb-4">
          <ArrowLeft className="w-4 h-4" /> 返回财务管理
        </button>
        <div className="text-center py-16 text-slate-400">账单不存在</div>
      </div>
    )
  }
    ...
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-400 mt-1">创建于 {fmtDate(record.created_at)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-6">
            <h2 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
              <FileText className="w-4 h-4 text-blue-500" /> 账单信息
            </h2>
            <div className="grid grid-cols-2 gap-y-4 gap-x-8">
              <div>
                <p className="text-xs text-slate-400 mb-1">账单号</p>
                <p className="text-sm font-medium text-slate-900">{record.record_number}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-1">类型</p>
                <p className="text-sm font-medium text-slate-900">{isAR ? '应收账款' : '应付账款'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-1">{isAR ? '客户' : '承运商'}</p>
                <p className="text-sm font-medium text-slate-900">{record.counterparty_name || '-'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-1">关联订单</p>
              ...
              <div>
                <p className="text-xs text-slate-400 mb-1">币种</p>
              ...
              {record.auto_generated && (
                <div className="col-span-2">
                  <span className="inline-flex items-center gap-1 text-xs text-green-600 bg-green-50 px-2 py-1 rounded-lg">
                    <CheckCircle className="w-3 h-3" /> 订单完成自动生成
                  </span>
                </div>
              )}
              {record.remarks && (
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-6">
            <h2 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
              <DollarSign className="w-4 h-4 text-green-500" /> 金额明细
            </h2>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-500">账单金额</span>
                <span className="text-lg font-bold text-slate-900">{fmt(record.amount)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-500">已付金额</span>
                <span className="text-sm font-medium text-green-600">{fmt(record.paid_amount)}</span>
              </div>
              <div className="border-t border-slate-100 pt-3 flex justify-between items-center">
                <span className="text-sm font-medium text-slate-700">未付余额</span>
          ...
          <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-6">
            <h2 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-amber-500" /> 时间信息
            </h2>
