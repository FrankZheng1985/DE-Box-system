...
import { Truck, Search, Plus, Eye, Edit, Star, ChevronLeft, ChevronRight, CheckCircle, AlertCircle, Download } from 'lucide-react'
...
export default function CarrierList() {
  ...
  const fetchCarriers = async () => {
    setLoading(true)
    try {
      const res = await api.get<ApiResponse<CarrierListResponse>>(
        `/carriers?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}`
      )
            ...
            <tbody>
              {loading ? (
                ...
                carriers.map(carrier => (
                  <tr key={carrier.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-all duration-200">
                    <td className="px-4 py-3">
                      <button
                        onClick={() => navigate(`/carriers/${carrier.id}`)}
                        className="text-sm font-medium text-orange-600 hover:text-orange-700 hover:underline truncate block max-w-full text-left"
                      >
