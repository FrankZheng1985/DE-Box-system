...
### DNS 记录：决定保留不动（2026-08-02 Frank 定）

Strato 账号进不去，删不了。经评估**可以不删**：

- A / www 仍解析到 `47.83.241.117`，MX 仍指 `smtpin.rzone.de`
- 网站层面无影响——服务器已拒绝服务，访问者只看到连接错误
- IP 仍是自己的，别人拿不到，**不存在接管风险**
- 域名到期不续费会自动释放，届时解析自然消失

🚨 **但释放或更换这台 ECS 之前，必须先处理这条 DNS。**
阿里云会回收 IP 再分配，届时旧域名还指着它的话，新 IP 持有者可用该域名
提供任意内容，并能通过 HTTP-01 验证合法申请到 Let's Encrypt 证书做带绿锁的
假站（domain takeover）。风险不是现在有，而是换服务器那天突然成立。

老客户发到 `info@box-cargo.de` 的邮件会进 Strato 且收不到；但该域名邮件链路
早已因 SPF/DKIM 缺失被丢弃（踩坑 012），本来就不通，无新增损失。
